...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Narrator:	According to all known laws of aviation, there is no way that a bee should be able to fly. Its wings are too small to get its fat little body off the ground. The bee, of course, flies anyway because bees dont care what humans think is impossible. cut to Barrys room, where hes picking out what to wear
  		Barry Yellow, black. Yellow, black. Yellow, black. Yellow, black. Ooh, black and yellow! Yeah, lets shake it up a little.
  		Mom (Janet Benson)	(calling from downstairs:) Barry! Breakfast is ready!
   		...
   		Barry:	I cant believe it. Ill pick you up. (hangs up, sharpens his stinger) Lookin sharp. (flies downstairs)
   		Mom:	Barry, why dont you use the stairs? Your father paid good money for those.
  		Barry:	Sorry. Im excited.
  		Dad (Martin Benson):	Heres the graduate. Were very proud of you, son. And a perfect report card, all Bs.
  		Mom:	Very proud. (touches Barrys hair)
  		Barry: Ma! I got a thing going here.
   		Mom:Ah, you got some lint on your fuzz.
   		Barry:	Ow! Thats me!
   		Dad:Wave to us! Well be in row 118,000.', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var screenNoteIntensity:Float = 0.2;
	...
	var bgSkyAlt:BGSprite;
	var bgStreetAlt:BGSprite;
	var bgSchoolAlt:BGSprite;
	var fgTreesAlt:BGSprite;
	var bgAlt:FlxTypedGroup<FlxSprite>;
	var eteledTrail:FlxTrail;
	var evilSchool:BGSprite;
	var end:BGSprite;
	var dark:BGSprite;
	var endAlt:BGSprite;
	var effectBf:BGSprite;
	var effectDad:BGSprite;
	var warning1:FlxText;
	var warning2:FlxText;
	var lineMonster1:FlxText;
	var lineMonster2:FlxText;
	var lineMonster3:FlxText;
	var lineMonster4:FlxText;
	var lineMonster5:FlxText;
	var line1:FlxText;
	var line2:FlxText;
	var line3:FlxText;
	var line4:FlxText;
	var line5:FlxText;
	var vignette:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('Vignette', 'shared'));
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'school': //Week 6 - Senpai, Roses
				...
				bgAlt = new FlxTypedGroup<FlxSprite>();
				add(bgAlt);

				bgSkyAlt = new BGSprite('weeb/weebSkyEvil', 0, 0, 0.1, 0.1);
				bgAlt.add(bgSkyAlt);
				bgSkyAlt.alpha = 0;
				bgSkyAlt.antialiasing = false;

				var repositionShit2 = -200;

				bgSchoolAlt = new BGSprite('weeb/weebSchoolEvil', repositionShit2, 0, 0.6, 0.90);
				bgAlt.add(bgSchoolAlt);
				bgSchoolAlt.alpha = 0;
				bgSchoolAlt.antialiasing = false;

				bgStreetAlt = new BGSprite('weeb/weebStreetEvil', repositionShit2, 0, 0.95, 0.95);
				bgAlt.add(bgStreetAlt);
				bgStreetAlt.alpha = 0;
				bgStreetAlt.antialiasing = false;

				var widShit2 = Std.int(bgSkyAlt.width * 6);
				fgTreesAlt = new BGSprite('weeb/weebTreesBackEvil', repositionShit2 + 170, 130, 0.9, 0.9);
				fgTreesAlt.setGraphicSize(Std.int(widShit2 * 0.8));
				fgTreesAlt.updateHitbox();
				bgAlt.add(fgTreesAlt);
				fgTreesAlt.alpha = 0;
				fgTreesAlt.antialiasing = false;

				bgSkyAlt.setGraphicSize(widShit2);
				bgSchoolAlt.setGraphicSize(widShit2);
				bgStreetAlt.setGraphicSize(widShit2);

				bgSkyAlt.updateHitbox();
				bgSchoolAlt.updateHitbox();
				bgStreetAlt.updateHitbox();
			case 'schoolEvil': //Week 6 - Thorns
				...
				var posX = 400;
				var posY = 200;
				evilSchool = new BGSprite('weeb/animatedEvilSchool', posX, posY, 0.8, 0.9, ['background 1'], true);
				evilSchool.animation.addByPrefix('glitch', 'background 2', 24, true);
				evilSchool.animation.addByPrefix('background 1', 'background 1', 24, true);
				evilSchool.scale.set(6, 6);
				evilSchool.antialiasing = false;
				add(evilSchool);
			case 'end': //Week 7 - Discharge //marked as: "the end"
			end = new BGSprite('final', -700, -400, 0.9, 0.9);
			end.antialiasing = true;
			add(end);

			endAlt = new BGSprite('final2', -700, -380, 0.9, 0.9);
			endAlt.antialiasing = true;
			endAlt.alpha = 0;
			add(endAlt);

			effectBf = new BGSprite('effect', 700, 170, 0.9, 0.9);
			effectBf.antialiasing = true;
			effectBf.scale.set(1.7,1.7);
			effectBf.alpha = 0;

			effectDad = new BGSprite('effect2', 0, 59, 0.9, 0.9);
			effectDad.antialiasing = true;
			effectDad.scale.set(2.5,2.5);
			effectDad.alpha = 0;
			case 'darkness': //not a week lmao //marked as: "you expected a slur lmfao"
			dark = new BGSprite('BackgroundFirst', -140, -140, 0.9, 0.9);
		...
		if (SONG.song.toLowerCase() == 'yoursong'){
			var random = FlxG.random.int(1,50);
			switch (random){
				case 1:
					dad.curCharacter = 'yourcharacter';
				case 29:
					dad.curCharacter = 'yourcharacter';
			}
		}
		...
		if(gfVersion == null || gfVersion.length < 1) {
			switch (curStage)
			{
				...
				case 'school' | 'schoolDark' |'schoolEvil':
					gfVersion = 'gf-pixel';
				default:
					gfVersion = 'gf';
			}
		...
		switch(curStage)
		{
			...
			case 'darkness':
				eteledTrail = new FlxTrail(dad, null, 2, 24, 0.2, 0.069); //nice
				insert(members.indexOf(dadGroup) - 1, eteledTrail);
		}
		...
		add(scoreTxt);
		lineMonster1 = new FlxText(0, 0, FlxG.width, "How long has it been?", 20);
		lineMonster1.setFormat(Paths.font("Gabriola.ttf"), 120, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		lineMonster1.alpha = 0;
		lineMonster1.screenCenter();

		lineMonster2 = new FlxText(0, 0, FlxG.width, "Days? Weeks? Months?", 20);
		lineMonster2.setFormat(Paths.font("Gabriola.ttf"), 120, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		lineMonster2.alpha = 0;
		lineMonster2.screenCenter();

		lineMonster3 = new FlxText(0, 0, FlxG.width, "And you are still AWAKE.", 20);
		lineMonster3.setFormat(Paths.font("Gabriola.ttf"), 120, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		lineMonster3.alpha = 0;
		lineMonster3.screenCenter();

		lineMonster4 = new FlxText(0, 0, FlxG.width, "Remember,", 20);
		lineMonster4.setFormat(Paths.font("Gabriola.ttf"), 120, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		lineMonster4.alpha = 0;
		lineMonster4.screenCenter();

		lineMonster5 = new FlxText(0, 0, FlxG.width, "Your loyalty.", 20);
		lineMonster5.setFormat(Paths.font("Gabriola.ttf"), 120, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		lineMonster5.alpha = 0;
		lineMonster5.screenCenter();

		line1 = new FlxText(0, 0, FlxG.width, "Missed me?", 20);
		line1.setFormat(Paths.font("cambriab.ttf"), 60, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		line1.alpha = 0;
		line1.screenCenter();
		line2 = new FlxText(0, 0, FlxG.width, "Ready for round 2?", 20);
		line2.setFormat(Paths.font("cambriab.ttf"), 60, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		line2.alpha = 0;
		line2.screenCenter();
		line3 = new FlxText(0, 0, FlxG.width, "Come on!", 20);
		line3.setFormat(Paths.font("cambriab.ttf"), 60, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		line3.alpha = 0;
		line3.screenCenter();
		line4 = new FlxText(0, 0, FlxG.width, "Keep your focus.", 20);
		line4.setFormat(Paths.font("cambriab.ttf"), 60, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		line4.alpha = 0;
		line4.screenCenter();
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + SONG.speed + ")", iconP2.getCharacter());
		#end
		add(lineMonster1);
		lineMonster1.cameras = [camHUD];
		add(lineMonster2);
		lineMonster2.cameras = [camHUD];
		add(lineMonster3);
		lineMonster3.cameras = [camHUD];
		line4.cameras = [camHUD];
		add(lineMonster4);
		lineMonster4.cameras = [camHUD];
		add(lineMonster5);
		lineMonster5.cameras = [camHUD];
		add(line1);
		line1.cameras = [camHUD];
		add(line2);
		line2.cameras = [camHUD];
		add(line3);
		line3.cameras = [camHUD];
		add(line4);
		if (curStage == 'end'){
			healthBar.alpha = 0;
			healthBarBG.alpha = 0;
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + "(" + SONG.speed + ")", iconP2.getCharacter(), true, songLength);
		#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + SONG.speed + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + SONG.speed + ")", iconP2.getCharacter());
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + SONG.speed + ")", iconP2.getCharacter());
		}
		#end
	...
	function doDeathCheck() {
		if (health <= 0 && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + SONG.bpm + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
		...
		if (!note.wasGoodHit)
		{
			switch(note.noteType) {
				case 'Hurt Note': //Hurt note
					...
					if(!boyfriend.stunned)
					{
						if(!endingSong)
						{
							...
							if(!note.isSustainNote) {
								camHUD.alpha -= 0.1; //loses 0.1 alpha per note hit
								new FlxTimer().start(5, function(tmr:FlxTimer){
									new FlxTimer().start(1, function(tmr:FlxTimer){
										camHUD.alpha += 0.1; //regives the alpha after 5 seconds
									});
								});
							}
							else
					...
					return;
				case 'Hit Note': //really difficult to win with these bad boys lmao
				//basically rebranded ebola notes lmao
					if(cpuControlled) return;
					...
					if(!boyfriend.stunned)
					{
						if(!endingSong)
						{
							if(!note.isSustainNote) {
								...
								health -= 0.2;	
								maxHealth += 0.1;	
								/*if (maxHealth == 0.9){ //runs a check everytime the note is hit (VERY UNOPTIMIZED BUT WORKS!!)
									maxHealth -= 0.1; //ensures the shit goes to normal instead of crashing the game (failsafe mechanic)
								}*/ //nvm lmao it fits because eteled funny crash
								doDrain();
								camHUD.shake(screenNoteIntensity, 0.2);	
								new FlxTimer().start(0.01, function(tmr:FlxTimer)
								{
									screenNoteIntensity -= 0.01;
								}, 20);		
								new FlxTimer().start(0.201, function(tmr:FlxTimer) //just resetting it so that the value stays the same for the next hit note (idk how it works if its negative)
								{ 
									screenNoteIntensity = 0.2;
								});	
								camHUD.flash(FlxColor.RED, 0.1);				
							}
							else
								dad.playAnim('attack', false);
								health -= 0.2;
								...
								/*if (maxHealth == 0.9){ //runs a check everytime the note is hit (VERY UNOPTIMIZED BUT WORKS!!)
									maxHealth -= 0.1; //ensures the shit goes to normal instead of crashing the game (failsafe mechanic)
								}*/ //nvm lmao it fits because eteled funny crasj
								doDrain();
								camHUD.shake(screenNoteIntensity, 0.2);	
								new FlxTimer().start(0.01, function(tmr:FlxTimer)
								{
	...
	function lineTextM1 () {
		FlxTween.tween(lineMonster1, {alpha: 1}, 0.2);
		new FlxTimer().start(3, function(tmr:FlxTimer){
			FlxTween.tween(lineMonster1, {alpha: 0}, 0.2);
		});
	}
	function lineTextM2 () {
		FlxTween.tween(lineMonster2, {alpha: 1}, 0.2);
		new FlxTimer().start(3, function(tmr:FlxTimer){
			FlxTween.tween(lineMonster2, {alpha: 0}, 0.2);
		});
	}
	function lineTextM3 () {
		FlxTween.tween(lineMonster3, {alpha: 1}, 0.1);
		new FlxTimer().start(2, function(tmr:FlxTimer){
			FlxTween.tween(lineMonster3, {alpha: 0}, 0.5);
		});
	}
	function lineTextM4 () {
		FlxTween.tween(lineMonster4, {alpha: 1}, 0.1);
		new FlxTimer().start(1.2, function(tmr:FlxTimer){
			FlxTween.tween(lineMonster4, {alpha: 0}, 0.1);
		});
	}
	function lineTextM5 () {
		FlxTween.tween(lineMonster5, {alpha: 1}, 0.1);
		new FlxTimer().start(2, function(tmr:FlxTimer){
			FlxTween.tween(lineMonster5, {alpha: 0}, 0.1);
		});
	}
	function lineText1 () {
		FlxTween.tween(line1, {alpha: 1}, 0.1);
		new FlxTimer().start(1, function(tmr:FlxTimer){
			FlxTween.tween(line1, {alpha: 0}, 0.1);
		});
	}
	function lineText2 () {
		FlxTween.tween(line1, {alpha: 1}, 0.1);
	...
	function lineText3 () {
		FlxTween.tween(line3, {alpha: 1}, 0.2);
		new FlxTimer().start(3, function(tmr:FlxTimer){
			FlxTween.tween(line3, {alpha: 0}, 0.2);
		});
	}
	function warning2Appear () {
		FlxTween.tween(warning2, {alpha: 1}, 0.5);
		new FlxTimer().start(5, function(tmr:FlxTimer){
			FlxTween.tween(warning2, {alpha: 0}, 2);
		});
	}
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'roses' && curStep == 257){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 263){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 393){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 396){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 409){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 412){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 445){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 448){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 513){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 521){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 673){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 687){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 777){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 785){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 793){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 797){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 801){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 817){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 829){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 831){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 897){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 906){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 969){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 972){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 989){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 991){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 1052){
			darkSwap1();
			camHUD.visible = false;
		}
		if (SONG.song.toLowerCase() == 'roses' && curStep == 1055){
			darkSwap2();
			camHUD.visible = true;
		}
		if (SONG.song.toLowerCase() == 'thorns' && curStep == 454){
			evilSchool.animation.play('glitch', true);
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 15){
			lineTextM1();
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 83){
			lineTextM2();
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 141){
			lineTextM3();
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 1105){
			FlxTween.tween(vignette, {alpha: 0.8}, 1);
			for (notes in playerStrums){
				FlxTween.tween(notes, {alpha: 1}, 1);
			}
			for (notes in strumLineNotes){
				FlxTween.tween(notes, {alpha: 1}, 1);
			}
		...
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 2737){
			lineTextM4();
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 2769){
			lineTextM5();
			FlxTween.tween(effectBf, {alpha: 1}, 2);
			FlxTween.angle(effectBf, 0, 360, 12);
			new FlxTimer().start(12, function(tmr:FlxTimer){
				FlxTween.angle(effectBf, 0, 360, 12);
			}, 200);
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 3025){
			FlxTween.tween(effectDad, {alpha: 1}, 2);
			FlxTween.angle(effectDad, 0, 360, 12);
			new FlxTimer().start(12, function(tmr:FlxTimer){
				FlxTween.angle(effectDad, 0, 360, 12);
			}, 200);
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 3636){
			FlxTween.tween(effectDad, {alpha: 0}, 2);
			FlxTween.tween(effectBf, {alpha: 0}, 2);
			FlxTween.tween(endAlt, {alpha: 0}, 2);
			FlxTween.tween(vignette, {alpha: 0}, 2);
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 1488){
			lineText1();
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 2641){
			FlxTween.tween(endAlt, {alpha: 1}, 1.2);
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 4405){
			camHUD.flash(FlxColor.WHITE, 1);
		}
		if (SONG.song.toLowerCase() == 'discharge' && curStep == 4406){
			camHUD.fade(FlxColor.BLACK, 0.1);
		}
		if (SONG.song.toLowerCase() == 'infinite' && curStep == 1787){
			remove(eteledTrail);
